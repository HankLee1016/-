...
from services.rag import LocalRAGRetriever, ingest_file, ingest_directory
from subsidy_db import fetch_subsidies, sync_subsidies_from_json, ensure_subsidies_table
from admin_tools import refresh_subsidies, refresh_service_applications
from service_applications_db import (
    create_service_application,
    get_service_application,
    get_user_service_applications,
    load_service_applications,
    update_service_application_status,
    ensure_service_applications_table,
)
# 這裡先保留檔案匯入函式，讓管理員可以把 PDF / TXT / HTML 直接變成知識庫文件。

app = Flask(__name__, template_folder="templates")
app.secret_key = os.getenv("SECRET_KEY", "dev_secret_key")

# 將功能型路由拆成獨立藍圖，避免 app.py 繼續膨脹。
...
# 單一 RAG 檢索器：先把補助資料放進本地知識庫，後續可替換成向量資料庫。
RAG_RETRIEVER = LocalRAGRetriever()
# 上傳檔案時可直接轉成知識庫文件；批次資料也可透過資料夾匯入。
...
# --- Subsidies ---
def load_subsidies():
    # 優先從資料庫讀取；若資料庫尚未建立，則回退到 subsidies.json。
    return fetch_subsidies()


def save_subsidies(subsidies):
    # 這裡保留舊 JSON 寫入能力，避免既有流程被直接切斷。
...
def _rag_ingest_subsidies():
    """把補助資料轉成知識庫文件。

    現在會優先從資料庫讀補助資料；若資料庫還沒準備好，
    `load_subsidies()` 會回退到 JSON，因此這段流程能同時支援兩種資料來源。
    """
    from services.rag.ingest import ingest_text

    total_docs = 0
    for subsidy in load_subsidies():
        # 將補助欄位整理成可檢索的純文字，讓 RAG 可以先工作。
        text = "\n".join([
            f"標題：{subsidy.get('title', '')}",
            f"機關：{subsidy.get('agency', '')}",
            f"分類：{subsidy.get('category', '')}",
            f"描述：{subsidy.get('description', '')}",
            f"資格：{subsidy.get('eligibility', '')}",
            f"截止日：{subsidy.get('deadline', '')}",
        ])
        docs = ingest_text(
            text,
            str(subsidy.get("id", subsidy.get("title", "subsidy"))),
            {"type": "subsidy", "title": subsidy.get("title", ""), "category": subsidy.get("category", ""), "source": subsidy.get("source_type", "seed")},
        )
        total_docs += RAG_RETRIEVER.ingest_documents(docs)
    return total_docs
...
# --- Applications ---
# 申請資料已改成「資料庫優先、JSON 備援」；舊 JSON 函式保留作為遷移期兼容，不再是主要來源。
def load_applications():
    return load_service_applications()
...
def create_application(username, case_title, background, issues, goals, proposal, subsidy_summary="", success_pdf=None, subsidy_pdf=None, status="pending"):
    return create_service_application(username, case_title, background, issues, goals, proposal, subsidy_summary, success_pdf, subsidy_pdf, status)


def get_application(application_id):
    return get_service_application(application_id)


def get_user_applications(username):
    return get_user_service_applications(username)


def update_application_status(application_id, status, admin_note=""):
    return update_service_application_status(application_id, status, admin_note)
...
# AI 模型與輔助功能
# ==========================================
# 這一段是專題中「智慧生成」的入口資訊，頁面會用它顯示目前模型與預算參考。
...
def api_generate_proposal():
    ...
    try:
        ...
        return jsonify({
            ...
            'ai_elapsed_ms': result.elapsed_ms,
            'ai_within_timeout': result.within_timeout,
        })
    except Exception as err:
        import traceback
...
def subsidies_page():
    ...
    if source_type:
        subsidies = [s for s in subsidies if s.get("source_type", "") == source_type]

    if sort_by == "deadline":
        subsidies = sorted(subsidies, key=lambda s: (s.get("deadline") or "9999-12-31", s.get("title", "")))
    elif sort_by == "title":
        subsidies = sorted(subsidies, key=lambda s: s.get("title", ""))
    elif sort_by == "agency":
        subsidies = sorted(subsidies, key=lambda s: s.get("agency", ""))

    all_subsidies = load_subsidies()
    categories = sorted({s.get("category", "其他") for s in all_subsidies})
    source_types = sorted({s.get("source_type", "seed") for s in all_subsidies})
    total_count = len(all_subsidies)
    visible_count = len(subsidies)
    return render_template(
        "subsidies.html",
        username=session.get("username"),
        subsidies=subsidies,
...
# ==========================================
@app.route("/admin/subsidies/refresh", methods=["POST"])
def admin_refresh_subsidies():
    if session.get("role") != "admin": return redirect(url_for("home"))
    ensure_subsidies_table()
    count = refresh_subsidies()
    flash(f"已同步 {count} 筆補助資料。", "success")
    return redirect(url_for("admin_dashboard"))

@app.route("/admin/applications/refresh", methods=["POST"])
def admin_refresh_service_applications():
    if session.get("role") != "admin": return redirect(url_for("home"))
    ensure_service_applications_table()
    count = refresh_service_applications()
    flash(f"已同步 {count} 筆申請資料。", "success")
    return redirect(url_for("admin_applications"))
...
def admin_dashboard():
    ...
    subsidy_total = len(load_subsidies())
